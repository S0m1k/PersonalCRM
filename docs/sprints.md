...
- [ ] Структура репозитория (`backend/`, `docs/`, `frontend/`)
- [ ] FastAPI-скелет, health-check эндпоинт
- [ ] Подключение к MongoDB (docker-compose для локалки)
- [ ] Конфиг через `.env` (секреты, строки подключения)
...
- [ ] Базовый UI: список контактов + карточка (создание/редактирование)
...
- Фронтенд: какой стек? (React / Vue / другое)
- Хостинг и домен (нужен для OAuth redirect и webhooks)
- Multi-user или single-user (влияет на auth и модель данных)
- Apple/iCloud как источник — нужен ли?
